...
class JoinSplitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        # All nodes should start with 1250 BTC:
        ...
        pool = [0, 1, 2, 3]
        for i in range(4):
            (total_in, inputs) = gather_inputs(self.nodes[i], 200)
            pool[i] = self.nodes[i].createrawtransaction(inputs, {})
            pool[i] = self.nodes[i].zcrawjoinsplit(pool[i], {}, {zcaddress:199.9}, 199.9, 0)
