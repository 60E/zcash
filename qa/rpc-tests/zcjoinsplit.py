...
class JoinSplitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        (total_in, inputs) = gather_inputs(self.nodes[0], 200)
        protect_tx = self.nodes[0].createrawtransaction(inputs, {})
        joinsplit_result = self.nodes[0].zcrawjoinsplit(protect_tx, {}, {zcaddress:199.9}, 199.9, 0)
        ...
        joinsplit_result = self.nodes[0].zcrawjoinsplit(joinsplit_tx, {receive_result["note"] : zcsecretkey}, {zcaddress: 199.8}, 0, 0.1)
